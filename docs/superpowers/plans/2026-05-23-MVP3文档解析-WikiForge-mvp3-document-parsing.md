...
| [ ] | R2-3 | Next | PDF 基础文本抽取，失败时返回明确原因 | T1 / T2 |
| [ ] | R2-4 | Later | Word `.docx` 基础文本抽取 | T1 / T2 |
...
- 其他格式暂保持 `pending`，不在本轮引入 PDFBox / POI。
...
- PDF / Word 仍为下一轮节点，本轮不引入 PDFBox / POI。
