...
MVP5 R4-2 已冻结 HTTP Preview 契约，详见：
...
- `search_sources`：按关键词、状态分页查询 Source / SourceFile 安全摘要。
- `get_source`：读取 Source 元数据、正文摘录和关联 Obsidian Note 摘要。
- `create_source`：写入文本型 Source 草案，不写文件系统，不自动抓取在线链接。
- `get_obsidian_note`：读取已登记 Obsidian Note 的 Vault 相对路径、URI 和 Markdown。
- `create_personal_record`：写入消费、账单、邮件、人际关系或个人事件记录，先只做结构化存储。

R4-4 已在 `wikiforge-core-service` 内落地 HTTP Preview 首批工具最小实现：

- `search_sources`、`get_source`、`create_source` 已启用。
- `get_obsidian_note`、`create_personal_record` 已启用。
- `get_obsidian_note` 只读取已登记 Obsidian Note，返回 Vault 相对路径、URI 和可选 Markdown，不暴露本地绝对路径。
- Note Markdown 读取前会校验最终真实路径仍在 Vault 根目录内，避免路径穿越和符号链接逃逸。
- `create_personal_record` 写入 `personal_records` 草案，初始状态为 `pending`，不自动触发 AI 总结或 Obsidian 写入。
- `mcp_tool_calls` 记录每次调用，敏感字段只记录摘要信息。
- `create_source` 创建文本型 Source 草案、伪 SourceFile、SourceContent 和导入任务记录，不写本地源文件，不抓取远程链接，不自动写 Obsidian。
...
| MCP | MVP5 先提供 HTTP Preview API，官方 Java SDK 作为后续 transport 替换方向 | Spring AI MCP 后续评估 | 不让 MCP 阻塞 MVP |
...
- R4-2 已冻结 API：`GET /api/v1/mcp/tools`、`POST /api/v1/mcp/tools/{toolName}/call`、`GET /api/v1/mcp/calls`。
- R4-3 已完成 `search_sources`、`get_source`、`create_source` 最小实现和调用日志落库。
- R4-4 已完成 `get_obsidian_note` 和 `create_personal_record` 最小实现、安全路径校验和日志脱敏验证。
- R4-5 继续补充 MCP 调用日志的 Web 展示和 OpenClaw / Hermes 本机接入说明。
