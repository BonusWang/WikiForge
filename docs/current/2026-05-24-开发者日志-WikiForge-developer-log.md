...
## 2026-05-23 R4-5 OpenClaw / Hermes 接入说明

本轮继续执行 R4-5-4，输出 OpenClaw / Hermes 本机接入 WikiForge MCP HTTP Preview 的可执行说明。
...
- 当前 WikiForge 提供的是 MCP HTTP Preview，不是完整 stdio / SSE MCP transport。
- OpenClaw / Hermes 第一版应通过 HTTP Tool、Custom Action、Connector 或 Bridge Script 接入。
- 本机默认 Base URL：`http://localhost:8080/api/v1/mcp`。
- Docker 外部容器访问宿主机时使用：`http://host.docker.internal:8080/api/v1/mcp`。
- 同一 Docker Compose 网络内访问时使用：`http://wikiforge-core-service:8080/api/v1/mcp`。
- `X-WikiForge-Caller-Type` 和 `X-WikiForge-Caller-Id` 只用于审计和 UI 展示，不作为权限凭证。

文档包含的最小调用样例：

- `GET /api/v1/mcp/tools`
- `POST /api/v1/mcp/tools/create_personal_record/call`
- `POST /api/v1/mcp/tools/create_source/call`
- `POST /api/v1/mcp/tools/search_sources/call`
- `POST /api/v1/mcp/tools/get_source/call`
- `POST /api/v1/mcp/tools/get_obsidian_note/call`
- `GET /api/v1/mcp/calls`

安全边界：

- 文档不写真实 token、真实密钥或用户提供的模型密钥。
- 当前无登录鉴权，只允许本机或可信内网使用，不应直接暴露公网。
- `create_personal_record` 会保存业务原文到 `personal_records`，适合本地私有部署；MCP 调用日志仍保持脱敏。

当前状态：

- R4-5-4 已完成。
- 当前执行指针移动到 R4-5-5：集成验证、文档归档、提交推送、合入 main、标签和发布。

## 2026-05-23 R4-5 集成验证与 0.08-preview.3 发布候选

本轮执行 R4-5-5：对 R4-5 MCP 调用看板、OpenClaw / Hermes 接入说明和 MVP5 HTTP Preview 主线做发布前收口。

本轮更新：

- `docs/current/2026-05-23-版本更新记录-WikiForge-release-notes.md` 新增 `0.08-preview.3`。
- Roadmap 将 S7 / R4 / MVP5 Orchestration + MCP 标记为 Done。
- R4-5-5 标记为 Done。
- 归档快照推进到 v4.1。
...
```text
mvn -s <temp-settings> "-Dmaven.repo.local=E:\repository" test
Result: pass, 5 modules, 52 tests, 0 failures
...
Secret scan
Result: no MiniMax secret prefix, Bearer token, or common API key pattern found

Forbidden tracked path scan
Result: no node_modules, dist, target, .env, data, Vault, or Raw Sources tracked
```
...
```text
Tag: 0.08-preview.3
Branch: codex/r4-5-mcp-dashboard-connectors
Scope: R4-5 MCP Dashboard + OpenClaw / Hermes Integration
```

当前状态：

- MVP5 HTTP Preview 主线完成。
- 下一操作为提交发布说明和最终归档快照，推送任务分支，合入 `main`，创建并推送 `0.08-preview.3` 标签和 GitHub Release。
- MVP 阶段完成后，后续产品路线进入 V1：在线资料连接器、个人记录持续整理、LifeOS 化记录和向量库准备。

## 2026-05-23 0.08-preview.3 发布完成

R4-5 最终发布收口已完成。

完成动作：

- 任务分支 `codex/r4-5-mcp-dashboard-connectors` 已推送到远程。
- `main` 已快进合并到提交 `e8c5bec` 并推送远程。
- 已创建并推送标签：`0.08-preview.3`。
- 已创建 GitHub prerelease：
  - https://github.com/BonusWang/WikiForge/releases/tag/0.08-preview.3

发布范围：

- MCP Preview Dashboard 只读看板。
- MCP calls 查询测试和 pageSize 100 对齐。
- OpenClaw / Hermes 本机 HTTP Preview 接入说明。
- Agent Team 文件化协作工作区和自动进入下一轮规则。
- Roadmap、开发者日志、MCP 契约、Release Notes 和归档快照。

当前结论：

- MVP5 HTTP Preview 主线已完成。
- MVP 当前阶段闭环完成，后续不继续自动进入 V1 编码；V1 涉及在线资料连接器、个人记录持续整理、LifeOS 化记录和向量库准备，需要下一轮需求切片和阶段计划。

## 2026-05-23 R4-6 MVP 审核加固

本轮响应“全面且具体审核 MVP，整理问题点并解决处理”的要求，对发布后的 MVP5 做一次安全边界、部署默认值、文档状态和发布状态自检。

发现问题：

- High：生产与开发 Compose 默认将 MySQL、Core、Worker、Orchestration、UI 端口绑定到所有网卡；MVP 当前无登录鉴权，默认对外暴露风险过高。
- Medium：`.env.example` 和 Orchestration Compose 默认 `WIKIFORGE_ACTIVE_BRANCH` 仍指向 `codex/mvp5-mcp-preview`，会误导 Orchestration UI 和后续 Agent。
- Medium：Roadmap、MCP 契约和 OpenClaw / Hermes 接入说明仍停留在 R4-5 发布状态，未记录发布后审核加固。
- Medium：`0.08-preview.3` 标签已发布后，`main` 又追加了发布记录提交；不应移动既有标签，需用新版本记录本轮加固。
- Low：OpenClaw / Hermes 文档未说明默认本机绑定后，Docker 外部容器访问宿主机端口的前提限制。

处理结果：

- `.env.example` 新增 `WIKIFORGE_PORT_BIND=127.0.0.1`，并将 `WIKIFORGE_ACTIVE_BRANCH` 改为 `main`。
- `deploy/docker-compose.yml` 和 `deploy/docker-compose.dev.yml` 的端口映射统一改为 `${WIKIFORGE_PORT_BIND:-127.0.0.1}:host:container`。
- OpenClaw / Hermes 接入说明补充同一 Compose 网络优先、显式开放端口和安全前提。
- MCP 契约新增 R4-6 审核加固记录。
- Roadmap 新增 R4-6 节点和 `0.08-preview.4` 收口说明。
- 新增 `docs/current/2026-05-23-MVP审核报告-WikiForge-mvp-audit-report.md`。
- 版本更新记录新增 `0.08-preview.4 - MVP 审核加固`。
...
docker compose -f deploy/docker-compose.yml config --quiet: pass
docker compose -f deploy/docker-compose.dev.yml config --quiet: pass
docker compose rendered ports: host_ip 127.0.0.1
mvn -s <temp-settings> "-Dmaven.repo.local=E:\repository" test with Java 21: pass, 5 modules, 52 tests, 0 failures
npm --prefix frontend run build: pass, existing Rollup chunk / PURE warnings only
npm --prefix orchestration-ui run build: pass
secret scan: pass, no MiniMax secret prefix, Bearer token, or common API key pattern found
forbidden tracked path scan: pass, no node_modules/dist/target/.env/data/Vault/Raw Sources tracked
```

发布策略：

- 不移动已发布的 `0.08-preview.3` 标签。
- 本轮验证通过后，以 `0.08-preview.4` 创建新的预览标签和 GitHub prerelease，作为 MVP5 审核加固版本。
