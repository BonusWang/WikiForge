...
- 当前阶段：S7 / R4 MVP5 Orchestration 辅助工程已完成，R4-3 Source 类 MCP Preview 工具已完成，下一节点为 Obsidian Note 读取和个人记录写入工具
- 当前测试门禁：T1 契约与单元测试
...
- (x) R4-4 实现 Obsidian Note 读取和个人记录写入工具
...
| [ ] | R4-4 | Doing | 实现 Obsidian Note 读取和个人记录写入 | T1 / T2 |
...
下一步：R4-4 实现 Obsidian Note 读取和个人记录写入工具。

## R4-2 完成结果

- 新增冻结契约：`docs/current/MCP接口契约-mcp-api-contract.md`。
- 冻结 HTTP Preview API：`GET /api/v1/mcp/tools`、`POST /api/v1/mcp/tools/{toolName}/call`、`GET /api/v1/mcp/calls`。
- 冻结首批工具：`search_sources`、`get_source`、`create_source`、`get_obsidian_note`、`create_personal_record`。
- 冻结权限边界：不返回本地绝对路径、不读取未登记文件、不执行删除/移动、不自动抓取在线文档、不自动调用 AI。
- 冻结 migration 编号：`V20260523_006__create_mcp_preview_tables.sql`。
- 下一步：R4-4 实现 Obsidian Note 读取和个人记录写入工具。
```
...
- 已新增 `mcp_tool_calls`、`personal_records` migration、MCP DTO、Service、Controller 和集成测试。
- 调用日志写入 MySQL，敏感输入只记录长度和 hash。
- `get_obsidian_note`、`create_personal_record` 仍为 disabled，进入 R4-4。
