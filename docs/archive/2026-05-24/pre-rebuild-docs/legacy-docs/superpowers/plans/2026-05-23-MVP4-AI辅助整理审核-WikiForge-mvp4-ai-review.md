...
| [x] | R3-5 | Done | 审核通过后更新 Obsidian Source Note | T2 / T3 |
| [x] | R3-6 | Done | 更新路线图、开发者日志、归档索引并提交推送 | T0 |
...
- `POST /api/v1/review-items/{reviewUid}/approve`
...
- 审核项必须处于 `pending` 才能通过，避免重复写入。
- 通过审核后复用 Obsidian Source Note 写入服务，把 AI Markdown 草案追加到 Source Note 中，并保存 `obsidian_notes`。
- 通过审核后更新 `review_items.status=approved`、`user_decision` 和 `reviewed_at`。
- Web UI 审核详情抽屉新增 `通过并写入 Obsidian` 按钮。
