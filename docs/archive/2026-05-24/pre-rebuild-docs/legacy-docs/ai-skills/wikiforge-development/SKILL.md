...
- MVP5 auxiliary split: `wikiforge-orchestration-service`, `wikiforge-orchestration-ui`.
...
WikiForge uses its own Orchestration auxiliary workflow inspired by Symphony:

- GitHub Issue or issue-style task card is the task control plane.
- `wikiforge-orchestration-service` exposes task, agent, workspace, verification, and handoff state.
- `wikiforge-orchestration-ui` shows the development control console.
...
- Do not copy OpenAI Symphony's Elixir server. WikiForge uses Java + Spring Boot + Vue for its auxiliary orchestration project.
- First orchestration version is read-only: no automatic local command execution and no automatic GitHub Issue mutation.
...
- Its Agent Team folder and own `STATUS.md`.
...
For parallel execution, the lead agent must create an `agentteam/{date}-{task-id}-{team-name}/` folder first. Each specialist agent must have its own folder with `README.md`, `PROMPT.md`, `SKILL.md`, `WORKSPACE.md`, and `STATUS.md`. The lead agent assigns disjoint file ownership, freezes shared contracts, then dispatches specialist agents. The lead agent owns final integration, verification, docs, archive updates, tags, and releases.
