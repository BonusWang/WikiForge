...
## 1. WikiForge Orchestration Task Control

WikiForge 采用自建 Orchestration 辅助工程模式：

- GitHub Issue 或 Issue 风格任务卡是任务控制平面。
- `wikiforge-orchestration-service` 是任务、Agent、工作区、验证和 Handoff 状态服务。
- `wikiforge-orchestration-ui` 是开发编排控制台。
...
- 当前不引入 Symphony Elixir 服务端。
- 第一版不自动执行本机命令，不自动修改 GitHub Issue。
