...
- 发布候选标签：`0.04`
- 对应路线图节点：R0-4
- 当前结论：MVP2 主链路已验证通过，开发分支已提交并推送；版本标签和 GitHub Release 是外部发布宣导事项，仍由用户确认，但不阻塞 R1/R2 内部开发递进
...
| [x] | R0-5 | Done | commit `a987780` 已推送到 `origin/codex/mvp2-obsidian-source-note` |
| [ ] | R0-6 | External Pending | 标签 `0.04` 和 GitHub Release 等待用户定义，不阻塞内部开发、提交推送和下一工作流递进 |
...
| [x] | T2 | 构建验证 | 通过 |
| [x] | T3 | Docker 节点烟测 | 通过 |
| [ ] | T4 | 阶段级发布验收 | 等待用户对外发布确认，不阻塞 R1 开发 |
...
- [ ] 用户复核当前 `0.04` 发布范围。
- [x] 执行 R0-5：提交并推送当前开发分支。
- [ ] 用户确认后再创建标签 `0.04` 和 GitHub Release。
- [x] 进入 R1：MVP2.1 可用性加固。
