...
- 文档版本：v0.7
- 当前阶段：MVP5 / R4-6 审核加固，MCP HTTP Preview 私有化部署边界补强
- 当前分支：`codex/mvp-audit-fixes`
- 适用范围：R4-6 的 MVP 审核报告、Docker 默认端口绑定和 OpenClaw / Hermes 接入说明修正
...
Compose 默认通过 `WIKIFORGE_PORT_BIND=127.0.0.1` 只绑定宿主机本机端口。Docker 外部容器访问宿主机端口时，必须确认该容器能访问宿主机 loopback 映射；更推荐把调用方放进同一 Compose 网络并使用服务名访问。只有在明确需要局域网或外部容器访问时，才将 `WIKIFORGE_PORT_BIND` 改为 `0.0.0.0`，并同步配置防火墙、反向代理鉴权或 IP allowlist。

说明文档仅使用示例 Header 和占位 UID，不写真实 token、密钥或用户私密信息。

## 15. R4-5 发布候选验证记录

R4-5 发布候选 `0.08-preview.3` 已完成以下验证：

- 后端 Maven 全量测试通过，5 个模块合计 52 个测试，0 失败。
- `McpPreviewApiIntegrationTests` 8 个测试通过，覆盖 MCP tools、Source 工具、Obsidian Note、Personal Record、调用日志和安全边界。
- 前端构建通过，MCP Preview 看板代码通过 `vue-tsc` 和 Vite build。
- `docker compose -f deploy/docker-compose.yml config --quiet` 通过。
- `docker compose -f deploy/docker-compose.dev.yml config --quiet` 通过。
- Git 空白检查、敏感信息扫描和禁止路径扫描通过。

## 16. R4-6 MVP 审核加固记录

R4-6 对 MVP5 发布后的私有化部署边界和文档状态做一次审核加固：

- Compose 生产与开发文件的公开端口统一改为 `${WIKIFORGE_PORT_BIND:-127.0.0.1}:host:container`，默认仅本机可访问。
- `.env.example` 新增 `WIKIFORGE_PORT_BIND=127.0.0.1`，并将 `WIKIFORGE_ACTIVE_BRANCH` 默认值从历史阶段分支改为 `main`。
- OpenClaw / Hermes 接入说明补充 Docker 外部容器访问限制和同一 Compose 网络优先建议。
- 发布标签 `0.08-preview.3` 不回写移动；本轮修复完成后使用新的 `0.08-preview.4` 记录审核加固结果。
