...
| MVP 3 | `source_contents` | Core Service 管理，Worker Service 解析并回写 | Markdown / TXT 正文抽取、hash、解析状态和 Source Note 摘录 |
| MVP 4 | `agent_runs`、`agent_steps`、`review_items` | Agent Service / Core Service | AI 辅助整理和审核队列 |
| MVP 5 | `mcp_servers`、`mcp_tool_calls` | MCP Service | 轻量 MCP 预览 |
| V1/V2 | `personal_records`、`content_chunks`、`embedding_jobs`、办公室视图相关表 | Record / Vector / Agent Service | 个人记录、向量库和运行层 |
...
保存 Worker 从源文件中抽取出的正文内容。MVP3 首轮只支持 Markdown / TXT，PDF / Word 在下一轮补充。
...
| parser_name | varchar(128) | `markdown-text`、`plain-text`，后续扩展 `pdfbox`、`poi-docx` |
