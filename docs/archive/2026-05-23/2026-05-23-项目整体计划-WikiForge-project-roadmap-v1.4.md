...
- 当前工程阶段：MVP5 WikiForge Orchestration 辅助工程骨架已完成；当前节点为 R4-2 MCP 工具清单和权限边界冻结
...
1. `AGENTS.md`
2. `docs/archive/2026-05-23/2026-05-23-归档索引-archive-index-v3.0.md`
3. 本文档
...
| R4-2 | Doing | 冻结 MCP 工具清单和权限边界 | 技术架构、数据模型、Work Order | 工具不越权，不直接暴露本地路径 |
| R4-3 | Todo | 实现 `create_source` / `search_sources` / `get_source` | MCP Preview API | 外部客户端可调用 |
| R4-4 | Todo | 实现 `get_obsidian_note` / `create_personal_record` | MCP Preview API | Agent 可读 Note、写个人记录 |
| R4-5 | Todo | 记录 MCP 调用日志并编写 OpenClaw / Hermes 接入说明 | MySQL / docs | 每次调用可追踪，本机可按说明接入 |
