# 2026-05-23 WikiForge 文档归档索引 v4.1
...
本轮完成 R4-5-1/2/3/4/5：Dashboard 新增 MCP Preview 只读看板，展示 MCP tools 清单和 calls 调用日志；前端通过 `frontend/src/api/mcp/` 和 `frontend/src/types/mcp.ts` 接入 API；后端补充 `GET /api/v1/mcp/calls` 查询端点集成测试，并将 MCP calls `pageSize` 上限对齐到 100；新增 OpenClaw / Hermes 接入说明，明确当前采用 HTTP Tool / Custom Action / Connector / Bridge Script 调用 MCP HTTP Preview，并提供本机、Docker 外部容器、Compose 网络内三种 Base URL 和 PowerShell 示例。发布候选验证通过：后端 Maven 全量测试 5 个模块合计 52 个测试，前端构建通过，生产与开发 Compose 配置通过，Git 空白、密钥和禁止路径扫描通过。`0.08-preview.3` 发布说明已补充；当前下一操作为提交推送、合入 `main`、创建并推送 `0.08-preview.3` 标签和 GitHub Release。MVP5 HTTP Preview 主线完成，后续产品路线进入 V1 在线资料连接器、个人记录持续整理、LifeOS 化记录和向量库准备。
