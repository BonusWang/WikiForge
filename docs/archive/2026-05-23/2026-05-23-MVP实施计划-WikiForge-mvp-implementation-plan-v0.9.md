# 2026-05-23 知识熔炉 WikiForge MVP 实施计划 MVP Implementation Plan v0.9
...
4. MVP 3：文档解析与 Source Note 丰富化。
5. MVP 4：AI 辅助整理与人工审核。
6. MVP 5：WikiForge Orchestration 辅助工程 + 轻量 MCP 预览。
...
- MVP 3 引入 `source_contents`，完成 Markdown / TXT / PDF / Word 基础正文抽取。
- MVP 4 引入 AI 辅助整理、`agent_runs`、`agent_steps`、`review_items` 和人工审核后写入 Obsidian。
- MVP 5 先完成 Orchestration 辅助工程，再提供轻量 MCP HTTP Preview；官方 MCP SDK 保留为后续 transport 替换方向。
...
当前用户确认的本机 Obsidian Vault 宿主机路径：

```text
E:\WikiForgeVault
```

MVP2 实施时，服务端运行路径通过 `WIKIFORGE_OBSIDIAN_VAULT_PATH` 注入；Docker 模式通过 `WIKIFORGE_HOST_OBSIDIAN_VAULT_PATH` 将宿主机 Vault 挂载到容器内 `/data/wikiforge/obsidian-vault`。不得把 Vault 内容提交到 Git 或打进镜像。

当前实现状态：

- 已完成 Vault 初始化、Source Note 草案、写入 Vault、`obsidian_notes` 记录、Markdown 预览和 `obsidian://open` URI。
- 已完成 Web UI Source Note 抽屉。
- 已通过本地测试、前端构建、Docker Compose 重建、Core/Worker health 和浏览器 UI 检查。
- `90_System_系统/Indexes_索引/index.md` 和 `90_System_系统/AgentLogs_Agent日志/log.md` 暂未进入 MVP2，实现顺延到 Wiki 编译和维护 Agent 阶段。
...
### MVP 5：Orchestration 辅助工程 + 轻量 MCP 预览版

目标：先建立长期开发编排控制台，再为 OpenClaw / Hermes 接入打基础，不阻塞 MVP 0-4。

功能：

- Orchestration Service / UI 只读任务控制台。
- MCP HTTP Preview API。
- `create_source`
- `search_sources`
- `get_source`
- `get_obsidian_note`
...
- Orchestration UI 能展示任务状态。
- 外部 Agent 能通过 HTTP Preview 调用基础工具。
