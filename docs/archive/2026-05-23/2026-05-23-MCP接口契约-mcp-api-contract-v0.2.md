...
- 文档版本：v0.2
- 当前阶段：MVP5 / R4-3 Source 类 MCP Preview 工具实现完成
- 当前分支：`codex/mvp5-mcp-preview`
- 适用范围：R4-4、R4-5 的后端、前端、文档和测试实现
...
R4-3 已完成 Source 类 MCP Preview 工具的最小实现：
...
| `get_obsidian_note` | Listed / Disabled | 工具定义已在 allow list 中展示，实际读取能力进入 R4-4 |
| `create_personal_record` | Listed / Disabled | 工具定义已在 allow list 中展示，实际写入能力进入 R4-4 |

所有 R4-3 工具调用都会写入 `mcp_tool_calls`。`rawContent` 不写入调用日志原文，只记录长度和 SHA-256 摘要。
...
- `get_obsidian_note`、`create_personal_record` 保持 disabled，作为 R4-4 当前任务。
