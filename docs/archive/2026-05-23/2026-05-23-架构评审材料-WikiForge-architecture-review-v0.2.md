# 2026-05-23 WikiForge 架构评审材料 v0.2
...
- CI/CD、Docker 打包和部署方式是否从一开始就可行，避免后续因发布困难反改架构。
...
- 后端使用 Maven 构建 Spring Boot jar。
- 前端使用 Vite 构建静态资源。
- MVP 推荐前后端分离镜像：`wikiforge-backend`、`wikiforge-frontend`、`mysql:8`。
- 使用 Docker Compose 管理应用、前端和 MySQL。
- Raw Sources、Obsidian Vault、MySQL 数据、日志和配置必须通过 volume 或外部路径持久化。
- CI/CD 需要覆盖后端测试、前端构建、Docker 镜像构建和 Compose 基础启动校验。
...
### 8.5 CI/CD 与 Docker 发布作为架构约束

WikiForge 从 MVP 0 开始就需要考虑可自动构建和可容器化发布。

关键约束：

- 前后端分离构建，避免把部署问题混入业务代码。
- Docker 镜像不写死本机路径。
- Raw Sources、Obsidian Vault、MySQL 数据和日志必须外部挂载。
- 运行时配置通过环境变量或外部配置文件注入。
- Flyway migration 必须能在空库上自动执行。
- `/actuator/health` 必须可用于 Docker healthcheck 和 CI 启动校验。
- 前端 API 地址不能写死 `localhost`，应通过 Nginx 代理或环境配置解决。
...
### 9.7 CI/CD、Docker 与部署

- 当前前后端分离结构是否适合 CI/CD 自动构建？
- 是否应该采用 `backend/`、`frontend/`、`deploy/` 的仓库结构？
- MVP 是否采用前后端分离镜像，而不是把前端打进 Spring Boot jar？
- Docker Compose 是否需要同时管理 backend、frontend、mysql？
- Obsidian Vault 和 Raw Sources 在 Docker 模式下如何挂载最稳？
- CI 是否必须跑 `mvn test`、`npm run build`、Docker build 和 Compose smoke test？
- Flyway migration 在 Docker 启动时执行是否合理？
- 是否需要为 NAS / 私有服务器部署预留 `.env.example` 和配置模板？
