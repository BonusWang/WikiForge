...
- 当前阶段：S7 / R4 MVP5 轻量 MCP 预览
- 当前测试门禁：T0 文档与 Git 卫生

## 当前执行指针

- ( ) R4-0 升级 Symphony-inspired Agent 工作模式
- (x) R4-1 冻结 MCP 工具清单和权限边界
- ( ) R4-2 实现 `create_source` / `search_sources` / `get_source`
- ( ) R4-3 实现 `get_obsidian_note` / `create_personal_record`
- ( ) R4-4 记录 MCP 调用日志
- ( ) R4-5 编写 OpenClaw / Hermes 接入说明
- ( ) R4-6 文档归档、验证、提交推送
...
| [ ] | R4-1 | Doing | 冻结 MCP 工具清单、权限边界和 Parallel Work Order | T0 |
| [ ] | R4-2 | Todo | 实现 Source 查询类工具和轻量调用入口 | T1 / T2 |
| [ ] | R4-3 | Todo | 实现 Obsidian Note 读取和个人记录写入 | T1 / T2 |
| [ ] | R4-4 | Todo | 落地 `mcp_servers`、`mcp_tool_calls` 调用日志 | T1 |
| [ ] | R4-5 | Todo | 补充 OpenClaw / Hermes 本机接入说明 | T0 |
| [ ] | R4-6 | Todo | 更新路线图、开发者日志、归档索引并提交推送 | T3 |
...
- 依赖任务：PWO-R4-1
- 输入契约：`GET /api/v1/mcp/tools`、`POST /api/v1/mcp/tools/{toolName}/call`
- 输出契约：工具列表、工具调用、调用日志
- 验证命令：`mvn -B -pl wikiforge-core-service -am -Dtest=McpPreviewApiIntegrationTests test`
- 是否可并行：可与 UI Agent 并行，但 Flyway migration 属于主编排串行区
- 合并顺序：第二
- Handoff 要求：说明实际新增 DTO、表、接口和测试

### PWO-R4-3 UI / Dashboard
