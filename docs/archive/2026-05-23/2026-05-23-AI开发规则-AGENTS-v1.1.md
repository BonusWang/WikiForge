...
- 每次提交信息必须简要说明本次改动点。
- 版本标签、GitHub Release 和正式版本发布定义必须等待用户确认后再执行。
- 版本标签和 GitHub Release 属于对外宣导和发布定义，不阻塞项目内开发分支、提交推送和下一工作流递进。
- 后续 AI 不得把“标签/Release 待用户确认”识别为开发阻塞；只要开发节点已完成验证，就继续按计划提交、推送并进入下一工作流。
