# 2026-05-23 WikiForge 文档归档索引 v4.0
...
本轮完成 R4-5-1/2/3/4：Dashboard 新增 MCP Preview 只读看板，展示 MCP tools 清单和 calls 调用日志；前端通过 `frontend/src/api/mcp/` 和 `frontend/src/types/mcp.ts` 接入 API；后端补充 `GET /api/v1/mcp/calls` 查询端点集成测试，并将 MCP calls `pageSize` 上限对齐到 100；新增 OpenClaw / Hermes 接入说明，明确当前采用 HTTP Tool / Custom Action / Connector / Bridge Script 调用 MCP HTTP Preview，并提供本机、Docker 外部容器、Compose 网络内三种 Base URL 和 PowerShell 示例。验证结果：`McpPreviewApiIntegrationTests` 8 个测试通过，`npm --prefix frontend run build` 通过，浏览器检查 `http://127.0.0.1:3002/` 可看到 MCP Preview、工具清单和调用日志，且无“调用工具”入口。当前执行指针移动到 R4-5-5：集成验证、归档、提交推送、合入 main、标签和发布。
