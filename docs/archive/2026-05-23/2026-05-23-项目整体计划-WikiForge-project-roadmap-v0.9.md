...
- 当前工程阶段：MVP4 AI 辅助整理与审核首轮已实现；Agent 运行账本、审核队列、MiniMax 环境变量适配和 Dashboard 审核队列已完成；下一节点为审核通过后更新 Obsidian Source Note
...
1. `AGENTS.md`
2. `docs/archive/2026-05-23/2026-05-23-归档索引-archive-index-v2.7.md`
3. 本文档
...
| [ ] | S6 MVP4 AI 辅助整理 | In Progress | 接入国内模型，生成摘要、标签、分类建议和审核队列 | 0.07 | 人工审核后写入 Obsidian |
...
| R3-5 | Next | 审核通过后更新 Obsidian Source Note | Core Service | 人工确认后才改写知识层 |
...
- MiniMax 通过 `WIKIFORGE_MINIMAX_API_KEY`、`WIKIFORGE_MINIMAX_BASE_URL`、`WIKIFORGE_MINIMAX_MODEL` 环境变量接入；密钥不进入仓库。
- 未配置或调用失败时使用本地规则兜底，保障 MVP4 主链路可用。
- Web UI Source Files 行内增加 `AI 整理`入口，Dashboard 增加审核队列和草案查看抽屉。
