# 2026-05-23 WikiForge 文档归档索引 v3.8
...
当前已完成 `0.08-preview.1` 阶段预览发布准备、`main` 合并和分支治理规则补充，并在短生命周期任务分支 `codex/r4-4-mcp-obsidian-personal-record` 完成 R4-4。MVP5 R4-4 已启用 `search_sources`、`get_source`、`create_source`、`get_obsidian_note`、`create_personal_record` 五个 HTTP Preview 工具；`mcp_tool_calls` 调用日志、`personal_records` 写入、MCP DTO、Service、Controller 和集成测试已落地。

本轮进一步建立 `agentteam/2026-05-23-R4-5-MCP调用看板-mcp-dashboard-agent-team/`，将 `claude-agent-examples` 中的 Todolist、Subagent、Agent Team 思路落为 WikiForge 文件化协作机制：任务计划单点推进、临时只读侦察和固定角色工作区分层、每个 Agent 通过自己的 `STATUS.md` 异步回报。主Agent 目录已新增 `CONTEXT.md`，用于记录当前主 Agent 的身份、用户偏好、项目状态、已完成决策和下一步动作，防止会话丢失。R4-5 前端侦察结论为：首版优先展示 MCP tools 和 mcp calls 日志，默认不开放真实工具调用。后端侦察结论为：`GET /api/v1/mcp/calls` 已存在，但需要补查询端点集成测试；测试侦察结论为：R4-5 按 T0-T4 递进验证，发布候选再做完整 Docker 健康检查。下一内部开发节点为 R4-5：实现前端只读 MCP 调用展示、补 calls 查询测试，并补充 OpenClaw / Hermes 本机接入说明。
