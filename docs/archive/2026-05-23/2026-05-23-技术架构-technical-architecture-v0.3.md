...
- 将 MCP 调用映射到 Backend API、Import Service、Search Service、Agent Orchestrator 等内部服务。
...
| 文档解析 | Apache POI、Apache PDFBox、commonmark-java/flexmark-java | Apache Tika 可作为聚合解析层 | 先用明确依赖，Tika 后续评估 |
...
- 使用 Model Context Protocol 官方 Java SDK 实现轻量 MCP Server。
- 优先暴露 `create_source`、`search_sources`、`get_source`。
...
- `wikiforge-backend`：Spring Boot 后端镜像。
- `wikiforge-frontend`：Nginx 托管前端静态资源，并反向代理 `/api` 到后端。
...
- Raw Source Organizer。
- 文件扫描、复制、hash 去重。
- Source / SourceFile / ImportJob 数据写入。
- Web UI 文件列表和导入任务状态。
...
- Model Provider Adapter。
- 单 LLM 多步骤处理。
- Normalize / Classify / Summarize / Generate Source Note。
- Review 队列。
- 人工确认后写入 Obsidian。
