# 2026-05-22 知识熔炉 WikiForge MVP 实施计划 v0.2

## 0. 架构评审后的执行调整

2026-05-23 架构评审后，MVP 执行顺序收敛为：

1. MVP 0：项目骨架 + CI/CD + Docker Compose + 基础配置 + Flyway 基线。
2. MVP 1：源文件归集。
...
- Docker Compose 可以启动 backend、frontend、mysql，并通过基础健康检查。
...
1. 后端项目骨架。
2. MySQL + Flyway 初始化。
3. system_settings / model_providers。
4. Raw Sources 和 Vault 路径配置。
5. Import Job + 文件扫描。
6. 文件复制归集 + hash 去重。
7. Source/File 列表 UI。
8. Obsidian Vault 初始化。
9. Source Note 模板和预览。
10. Review 队列。
11. Model Provider Adapter。
12. AI 辅助整理。
13. Obsidian 写入和打开。
14. MVP 验收测试。
