...
- 当前工程阶段：MVP5 轻量 MCP 预览已启动；R4-0 Symphony-inspired Agent 工作模式升级已完成，当前节点为 R4-1 MCP 工具清单和权限边界冻结
...
1. `AGENTS.md`
2. `docs/archive/2026-05-23/2026-05-23-归档索引-archive-index-v2.9.md`
3. 本文档
...
| [ ] | S7 MVP5 轻量 MCP | Doing | 让 OpenClaw / Hermes / 外部 Agent 能调用 WikiForge | 0.08 | MCP 工具可创建记录、查询 Source、读取 Note |
...
- (x) T3 Docker 节点烟测：完成跨服务节点或发布候选时使用。
...
| 节点 | 状态 | 事项 | 主要范围 | 验收 |
| --- | --- | --- | --- | --- |
| R4-0 | Done | 升级 Agent 工作流为轻量 Symphony-inspired 模式 | `WORKFLOW.md`、AGENTS、Skill、Issue Template、process docs | 后续 Agent 能按任务卡和 Work Order 接入 |
| R4-1 | Doing | 设计 MCP 工具清单和权限边界 | 技术架构、数据模型、Work Order | 工具不越权，不直接暴露本地路径 |
| R4-2 | Todo | 实现 `create_source` / `search_sources` / `get_source` | MCP Preview API | 外部客户端可调用 |
| R4-3 | Todo | 实现 `get_obsidian_note` / `create_personal_record` | MCP Preview API | Agent 可读 Note、写个人记录 |
| R4-4 | Todo | 记录 MCP 调用日志 | MySQL | 每次调用可追踪 |
| R4-5 | Todo | 编写 OpenClaw / Hermes 接入说明 | docs | 本机可按说明接入 |
