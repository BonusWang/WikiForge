# 2026-05-23 WikiForge 文档归档索引 v3.5
...
当前已完成 `0.08-preview.1` 阶段预览发布准备：MVP5 R4-3 Source 类 MCP Preview 工具已实现，`search_sources`、`get_source`、`create_source` 可用，`mcp_tool_calls` 调用日志、`personal_records` 预留表、MCP DTO、Service、Controller 和集成测试已落地。下一内部开发节点仍为 R4-4：实现 `get_obsidian_note` / `create_personal_record`。
