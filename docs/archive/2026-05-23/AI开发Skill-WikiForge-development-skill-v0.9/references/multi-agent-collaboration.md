...
WikiForge 使用自建 Orchestration 辅助工程模式。主编排 Agent 把用户目标拆成 Issue 风格任务卡，任务卡控制状态、owner、文件边界、验证命令和 Handoff；Work Order 控制实现顺序和契约；`wikiforge-orchestration-service` 和 `wikiforge-orchestration-ui` 负责长期展示和沉淀这些状态。

并行开发必须有一个主编排 Agent。主编排 Agent 负责：

- 维护 `WORKFLOW.md` 定义的任务状态。
- 将阶段节点拆成 GitHub Issue 或 Issue 风格任务卡。
- 拆解任务清单。
- 判断哪些任务可并行、哪些必须串行。
- 选择合适专家角色。
- 分配文件边界和禁止修改范围。
...
agentteam/{team}/README.md
agentteam/{team}/agents/{agent}/PROMPT.md
agentteam/{team}/agents/{agent}/SKILL.md
agentteam/{team}/agents/{agent}/WORKSPACE.md
对应 role prompt
对应 reference 文档
...
并行开发前必须先写 Issue 风格任务卡和 Parallel Work Order：
...
- `README.md` 描述团队目标和成员。
- `任务计划-team-plan.md` 保存复选框、单选执行指针、文件所有权和验证门禁。
- `PROMPT.md` 可直接复制给新会话。
- `SKILL.md` 是该 Agent 的角色技能和执行规则。
- `WORKSPACE.md` 是允许修改和禁止修改的文件范围。
- `STATUS.md` 是该 Agent 唯一可自主更新的状态交接文件。
- 专业 Agent 不修改正式 Roadmap、开发者日志、归档索引和 Release Notes。
...
主编排 Agent 收到 Handoff 后必须把任务状态更新为 Review 或 Done；如果仍有阻塞，更新为 Blocked，并写清楚下一步动作。
