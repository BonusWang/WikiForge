...
| 阶段 | 建表范围 | 说明 |
| --- | --- | --- |
| MVP 0 | `system_settings`、`model_providers` | 工程骨架、配置、模型供应商预留 |
| MVP 1 | `sources`、`source_files`、`import_jobs` | 本地路径扫描、复制归集、hash 去重、索引 |
| MVP 2 | `obsidian_notes` | Vault 初始化、Source Note 写入、Markdown 预览和打开 |
| MVP 3 | `source_contents`、`agent_runs`、`agent_steps`、`review_items` | AI 辅助整理和审核队列 |
| MVP 4 | `mcp_servers`、`mcp_tool_calls` | 轻量 MCP 预览 |
| V1/V2 | `personal_records`、`content_chunks`、`embedding_jobs`、办公室视图相关表 | 个人记录、向量库和运行层 |
