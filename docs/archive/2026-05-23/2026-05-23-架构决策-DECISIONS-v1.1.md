...
- MVP 3 再做文档解析。
- MVP 4 再做 AI 辅助整理。
- MVP 5 再做 Orchestration 辅助工程和轻量 MCP 预览。
...
- MVP 5：`mcp_tool_calls`、`personal_records`。
- V1/V2：`mcp_servers`、`content_chunks`、`embedding_jobs`、办公室视图相关表。
...
## 2026-05-23 Orchestration 辅助工程决策

用户确认 OpenAI Symphony 的工作模式需要在 WikiForge 中长期落地，但采用 WikiForge 自己的技术栈实现。

决策：

- 新增 `wikiforge-orchestration-service` 独立后端服务。
- 新增 `wikiforge-orchestration-ui` 独立前端控制台。
- 该辅助工程属于开发编排控制平面，不属于知识库核心业务服务。
- 不照搬 Symphony Elixir 服务端，不改变 WikiForge 主业务的 Java + Spring Boot + Vue 技术路线。
- 第一版只做只读任务清单、任务详情、状态统计和 UI 展示。
- 暂不自动执行 Codex / OpenClaw / Hermes 命令，暂不同步真实 GitHub Issue。

原因：

- 用户需要长期多人 / 多 Agent 迭代维护方式。
- 单靠 Markdown 计划和归档索引已经能工作，但状态可视化、任务边界、Handoff 和下一步动作需要一个独立可维护的控制台。
- 先做只读控制台可以快速落地，不把命令执行、安全隔离和 GitHub 权限复杂度提前塞进 MVP。

后续演进：

- GitHub Issue 双向同步。
- worktree / 分支创建和状态检测。
- Agent runner 注册、心跳、日志、失败重试。
- 验证命令执行记录和审计。
- 与 MCP / OpenClaw / Hermes 互通。
...
- 工具定义遵循 MCP 标准 tool schema 形态：`name`、`description`、`inputSchema`。
- 第一轮先提供 `/api/v1/mcp/tools` 和 `/api/v1/mcp/tools/{toolName}/call` HTTP Preview API，便于 OpenClaw / Hermes / 外部 Agent 本机调试。
- 官方 MCP Java SDK 作为后续 transport 替换方向保留；当前不为 MVP5 主闭环引入可能影响 Spring Boot 3.3 / Jackson 依赖管理的 SDK transport。
- 首批工具收敛为：`search_sources`、`get_source`、`create_source`、`get_obsidian_note`、`create_personal_record`。
- MCP 工具不直接返回本地绝对路径；本地文件、Source 和 Obsidian Note 通过 `sourceUid`、`fileUid`、`noteUid` 间接访问。

MVP5 顺序调整：

- 先完成 Orchestration 辅助工程骨架：`wikiforge-orchestration-service` + `wikiforge-orchestration-ui`。
- 再继续 MCP Preview API 和工具调用日志。
- Orchestration 与 MCP 是两个不同边界：Orchestration 管开发过程，MCP 管外部 Agent 调用 WikiForge 知识能力。

MVP5 R4-2 已确认：

- 冻结契约文档为 `docs/current/MCP接口契约-mcp-api-contract.md`。
- HTTP Preview API 为 `GET /api/v1/mcp/tools`、`POST /api/v1/mcp/tools/{toolName}/call`、`GET /api/v1/mcp/calls`。
- 首批工具最终冻结为：`search_sources`、`get_source`、`create_source`、`get_obsidian_note`、`create_personal_record`。
- `create_import_job`、`open_obsidian_note`、`get_review_items`、`search_personal_records` 暂不进入 MVP5 首批工具，后续按需要扩展。
- MVP5 migration 编号冻结为 `V20260523_006__create_mcp_preview_tables.sql`，先创建 `mcp_tool_calls` 和 `personal_records`。
- `mcp_servers` 不进入 MVP5 HTTP Preview 首个 migration，完整 MCP Server / Client 配置表后续再引入。
- 调用日志必须脱敏：`rawContent`、`markdown`、`structured` 不写入 `mcp_tool_calls` 原文。
