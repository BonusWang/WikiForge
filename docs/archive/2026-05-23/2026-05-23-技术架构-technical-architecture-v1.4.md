...
R4-3 已在 `wikiforge-core-service` 内落地 HTTP Preview 最小实现：

- `search_sources`、`get_source`、`create_source` 已启用。
- `get_obsidian_note`、`create_personal_record` 已在工具清单中展示但保持 disabled，进入 R4-4 实现。
...
- R4-4 继续实现 `get_obsidian_note` 和 `create_personal_record`。
