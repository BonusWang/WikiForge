...
敏感输入必须脱敏：`rawContent`、`markdown`、`structured` 不写入调用日志原文。
...
保存非文档型个人记录，例如消费、账单、邮件、人际关系和个人事件。MVP5 先通过 `create_personal_record` 做最小结构化写入，不做 AI 总结，不自动写 Obsidian。
