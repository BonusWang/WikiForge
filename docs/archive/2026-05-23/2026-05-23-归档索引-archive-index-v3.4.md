# 2026-05-23 WikiForge 文档归档索引 v3.4
...
当前已完成 MVP5 R4-3 Source 类 MCP Preview 工具，实现 `search_sources`、`get_source`、`create_source`，并落地 `mcp_tool_calls` 调用日志、`personal_records` 预留表、MCP DTO、Service、Controller 和集成测试。下一内部开发节点为 R4-4：实现 `get_obsidian_note` / `create_personal_record`。
