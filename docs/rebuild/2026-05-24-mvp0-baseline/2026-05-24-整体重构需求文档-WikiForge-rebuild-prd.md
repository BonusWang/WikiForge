...
- 资料仓库是事实源，只允许复制、移动或引用入库，不由 AI 修改原始资料。
...
- 本地散落文件默认预览后移动入库；浏览器上传默认复制入库；稳定资料可引用入库。
- 系统能识别重复文件并避免重复复制。
...
| 原子能力层 | 可复用最小能力 | 扫描、复制、移动、引用登记、hash、类型识别、正文抽取、Markdown 写入、路径安全、模型调用 |

## 5. 用户流程

### 5.1 路径扫描

1. 用户在收纳入口填写本机目录或单个文件。
2. 系统校验路径存在、未与资料仓库重叠。
3. Worker 扫描文件，跳过隐藏文件、临时文件和不安全路径。
4. 系统按入库策略复制、移动或引用登记资料。
...
- 资料仓库、Obsidian Vault、本地 `.env`、运行日志和数据库数据不得提交到 Git。
- 文件收纳按来源选择策略：浏览器上传复制，本地散落文件移动，稳定资料引用。
