...
| 决策层 | 规则、策略和 AI 整理决策 | 分类规则、重复策略、敏感/大文件策略、Wiki 写入规则、LLM 兜底策略 |
...
- 新表按需要新增，不提前为未来能力预建表。
- 现有历史表在后续数据库清理节点中评估迁移、退役或删除；本轮只写规则，不直接改 migration。
...
本轮不移动代码、不改 API、不改数据库。Orchestration 辅助开发工程先在文档中标记退役，后续清理节点再处理代码。
