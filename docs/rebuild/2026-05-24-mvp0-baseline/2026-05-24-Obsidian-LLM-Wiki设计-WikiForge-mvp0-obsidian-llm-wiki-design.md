...
| 路径 | 作用 |
| --- | --- |
| `index.md` | 总目录，展示来源页、主题页、项目页和最近写入 |
| `log.md` | 追加式演进日志 |
| `00_规则/` | LLM Wiki 写作规则和页面模板 |
| `10_来源/` | 每个 SourceFile 对应一个来源页 |
| `20_主题/` | 按主题沉淀知识页 |
| `30_项目/` | 按项目沉淀知识页 |
| `90_系统/` | 系统运行记录，不作为用户主阅读区 |
...
```text
WikiForge/20_主题/{主题名}.md
```

MVP0 默认存在：

```text
WikiForge/20_主题/待分类.md
```

如果 LLM 无法稳定判断主题，先写入“待分类”，不强行新建大量主题。
...
```text
WikiForge/30_项目/{项目名}.md
```

MVP0 默认存在：

```text
WikiForge/30_项目/待归档项目.md
```

项目归属不明确时，不强行归项目，只写来源页和主题页。
...
LLM 不可用、返回不合规或置信度不足时：

- 仍创建来源页。
- 主题页写入“待分类”。
- 摘要使用规则式正文截断。
- `wiki_ingest_runs.write_status_code` 记为“兜底写入”。
- `fallback_reason` 记录原因。

兜底写入不得阻断 Raw Sources 收纳结果。
...
| 模板渲染 | Core | 生成来源页、主题页、项目页内容 |
| 托管区块替换 | Core | 只更新系统托管内容 |
| index 更新 | Core | 重建或更新总目录 |
| log 追加 | Core | 追加演进记录 |
| 原子写入 | Core | 临时文件写入后原子替换 |
| LLM 整理 | Core | 可用时生成摘要、关键点、归属建议 |
| 规则式兜底 | Core | LLM 不可用时生成基础 Markdown |
